...
This document closes Phase 406. Every v41 HRN requirement — HRN-01 through HRN-08 — has a named home in one of §1–§6 above. The harness's three-layer physical decomposition, full plugin hook inventory, full MCP tool catalog, 4-tier intervention ladder, umbrella event schema, human-gate invariant, event-replay reconstruction proof, and full-Slice lifecycle sequence are all present, inlined where operative and cross-referenced where prose-only. No prior 402–405 spec was amended; the 12 prior canonical specs stay as-shipped, and this rollup is the consolidation lens that lets a reader see the whole harness at once without reading 480k of source specs first.

### HRN-01..08 → §1–§6 mapping table

| HRN | Subject | Owning section(s) | Operative contracts inlined | Cross-referenced upstream specs |
| --- | --- | --- | --- | --- |
| HRN-01 | 3-layer physical decomposition (hooks → state-build MCP → state-daemon) | §1 Layered Diagram | Layered Mermaid flowchart with named per-component arrows; cross-layer communication channels table; partition rule (sensors+enforcers vs agent-driven actions vs decision logic) | gsd-2 KB `cross-layer-communication-map.md` §1 (pattern source); PROJECT.md 6-component architecture (carry-forward) |
| HRN-02 | Plugin hook inventory (6 hooks: chat.params, chat.message, tool.execute.before/after, session.compacting, shell.env) | §2 Plugin Hook Inventory | Per-hook TS signature + firing trigger + role (inject/block/record) + v41 REQ cross-reference + source spec citation | 402 CONTEXT-PROTOCOL.md (CTX-02/06/08/09); 403 PLAN-AS-PROMPT.md (PAP-01/02/05/06); 404 PROOF-GATE.md (PRF-07 Layer 4); 404 SCOPE-PROHIBITION.md (SRP-02/04 Layers 1+3); 404 ANALYSIS-PARALYSIS-GUARD.md (bash classifier wiring); 405 SUBAGENT-MANAGEMENT.md (SUB-03/04 Layers 5+6); 405 DEVIATION-RULES.md (DEV-03 trailer wiring) |
| HRN-03 | state-build MCP tool catalog (14 tools w/ Pydantic input/output models) | §3 state-build MCP Tool Catalog | Per-tool: registration name, module path, input Pydantic class, output Pydantic class, semantics, plugin-hook integration site, owning v41 phase; `MCP_TOOL_REGISTRY` exhaustiveness with `assert_never` pattern | 402 CONTEXT-PROTOCOL.md (CTX-03/08 — `query_context_meter`, `request_compaction_snapshot`); 403 PLAN-AS-PROMPT.md (PAP-04 — `record_plan_edit`); 403/404 (`complete_task`, `complete_slice`, `check_proof_gate`); 404 SCOPE-PROHIBITION.md (SRP-05 — `request_step_split`, `scope_deviation_request`); 405 DEVIATION-RULES.md (DEV-03 — `log_deviation`); 405 SUBAGENT-MANAGEMENT.md (SUB-02 — `dispatch_subagent`) |
| HRN-04 | 4-tier intervention ladder (advisory → tool-block → clear+reinject → force-stop+human-gate) | §4 4-Tier Intervention Ladder | 4-tier overview; trigger-source enumeration per tier across APG/PRF/DEV/SUB/SCOPE/CTX chains; emission model (two events per intervention: per-chain + umbrella) | 404 ANALYSIS-PARALYSIS-GUARD.md §"6-advisory ladder" (APG triggers); 404 PROOF-GATE.md §"6-strike ladder" (PRF triggers); 405 DEVIATION-RULES.md §"4-rule deviation framework" (DEV triggers); 405 SUBAGENT-MONITORING.md §"Crash taxonomy + restart counter" (SUB triggers); 404 SCOPE-PROHIBITION.md (SCOPE triggers); 402 CONTEXT-PROTOCOL.md §"threshold action table" (CTX triggers) |
| HRN-05 | `state.harness.intervention` umbrella event (Pydantic schema + tier dispatcher) | §4 (4.3 + 4.4) | `HarnessIntervention` Pydantic class verbatim with 4-value `tier` Literal + 18-value `trigger_reason` Literal; deterministic dispatcher `dispatch_to_umbrella(source_type, source_payload)` with `assert_never` exhaustiveness; pure-machine discipline | 406 §1 (pure-machine discipline carry-forward from PRF-04 spirit); 405 SUB-06 (`assert_never` registry pattern); v40 EVENT-TAXONOMY.md (event naming convention) |
| HRN-06 | Human-gate-only-via-opencode-`question` structural invariant | §4.5 | CI grep enforcement (zero `prompt(`/`input(`/`Confirm(`/`Inquirer`/etc. allowed in `state_build/harness/intervention/`); acceptable surface (the single `surface_human_gate.py` calling opencode plugin's question-tool wrapper); absence-of-bypass carry-forward from DEV-04 | 405 DEVIATION-RULES.md DEV-04 (absence-of-bypass pattern source); 405 SUBAGENT-MONITORING.md §"Persistent orphan" (the `[abort_slice, retry_subagent, manual_resolve]` pre-authored alternatives example) |
| HRN-07 | Event-replay reconstruction proof | §5 Event-Replay Reconstruction Proof | §5.1 5-category replay event enumeration (38 event types — Slice-stage 4 + plan-lifecycle 9 + counter chains 17 + scope 4 + umbrella+context 4); §5.2 5-step reconstruction protocol; §5.3 daemon-restart worked example (hard case with 2 in-flight subagents + APG counter mid-chain); restart Mermaid sequenceDiagram; protocol invariants (deterministic, O(N+K), snapshot-optional, synthetic-event traceable, pure-machine) | 402 SLICE-CYCLE.md (Slice-stage events); 402 CONTEXT-PROTOCOL.md §5 CTX-05 (CompactionSnapshot Pydantic — replay rehydration source); 403 STEP-EVENTS.md (plan-lifecycle events + hash-chain integrity check); 404 PROOF-GATE.md + ANALYSIS-PARALYSIS-GUARD.md (PRF + APG counter chains); 404 SCOPE-PROHIBITION.md (SCOPE chain); 405 DEVIATION-RULES.md (DEV chain); 405 SUBAGENT-MONITORING.md SUB-08 (in_flight_subagents + restart_counters extension fields; orphan reconciliation 6-step flow) |
| HRN-08 | Full-Slice lifecycle Mermaid sequence diagram | §6 Full-Slice Lifecycle Sequence Diagram | §6.1 single Mermaid sequenceDiagram walking compaction-snapshot-schema Slice across all 4 stages (design-slice → research-slice → run-slice with 1 Step containing 3 tasks → verify-slice → close); 35+ event rows emitted; 4 tier-interventions named (Tier-1 APG advisory; Tier-2 PAP-05 plan_edit_blocked; Tier-3 APG paralysis_chain_3_reinject + compaction.snapshot_taken; Tier-4 STP-05 checkpoint:decision via surface_human_gate); §6.2 per-stage event tally + tier-call-out table + HRN-06 verification + cross-references | 403 EXEMPLAR-stepNPLAN.md (the canonical exemplar Slice); 402 SLICE-CYCLE.md (4-stage cycle structure); 402 CONTEXT-PROTOCOL.md (`compaction.snapshot_taken` + `compaction.reinject_completed` at tier-3 site); 403 STEP-EVENTS.md (plan_authored + plan_edit_blocked + checkpoint_human_action_*); 404 PROOF-GATE.md + ANALYSIS-PARALYSIS-GUARD.md + SCOPE-PROHIBITION.md (run-slice intervention sites); 405 SUBAGENT-MANAGEMENT.md + SUBAGENT-MONITORING.md (subagent spawns + progress + completion) |

### Phase-close discipline

- **No amendments to prior v41 specs.** The 12 canonical 402–405 specs stay as-shipped. Phase 405 already amended v40 EVENT-TAXONOMY.md / ARTIFACT-CATALOG.md / FRONTMATTER-SCHEMAS.md; Phase 406 does not extend that amendment chain. Forward-cross-references in 402–405 specs that mention "HRN-04/05 (Phase 406)" remain valid; no edit required.
- **Drift detection.** Any drift between an inlined operative contract (Pydantic schema, MCP tool signature, event payload, intervention dispatcher case) and its upstream source spec is a documented defect, detectable by grep on the literal contract string. The drift-as-defect rule was established by Phase 405 SUBAGENT-MANAGEMENT/MONITORING and carries forward verbatim.
