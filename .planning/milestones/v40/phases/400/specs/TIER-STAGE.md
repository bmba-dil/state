...
| `in_progress` | `verified` | `state.stage.slices_shipped` | ALL child Slices are `shipped` (D-19 composite check by projector) |
...
| `auditing` | `abandoned` | `state.stage.abandoned` | Explicit abandon command |

**Composite state note (D-19):** The Stage is `in_progress` when any child Slice is active (projector-computed). The transition to `verified` requires ALL child Slices to be `shipped` — this is checked by the projector as a composite condition. The `auditing` state is entered after a manual audit is initiated. The Stage ships only after the audit passes.
...
| `status` | `Literal["planned", "in_progress", "verified", "auditing", "shipped", "abandoned"]` | Projector | Yes | Current state, computed from event stream. Default: `"planned"` |
...
| `state.stage.slices_shipped` | Projector detects all child Slices `shipped` | `in_progress` → `verified` | Composite check (D-19): ALL child Slices are `shipped` |
| `state.stage.audited` | Manual audit initiated | `verified` → `auditing` | Formal audit initiated for the Stage |
| `state.stage.completed` | Audit passes | `auditing` → `shipped` | Audit passes + STAGE-SUMMARY.md produced |
| `state.stage.verification_failed` | Audit fails | `auditing` → `verified` | Return to `verified` for remediation |
...
The v40 design expands the Stage event set from 4 to 8 events. Some old events do not have a 1:1 mapping — the design splits verification and auditing into distinct events.
...
| `state.phase.verified` | `state.stage.slices_shipped` | Old single "verified" event split in v40: `slices_shipped` transitions to `verified` state; `audited` transitions to `auditing` state; `completed` transitions from `auditing` to `shipped` |
| `state.phase.completed` | `state.stage.completed` | Renamed aggregate. v40 design: triggers `auditing` → `shipped` transition after audit passes |
| — | `state.stage.audited` | New event: initiates the `verified` → `auditing` transition (formal audit) |
| — | `state.stage.verification_failed` | New event: returns from `auditing` → `verified` when audit fails |
