...
<objective>
Add event factory helper functions for ergonomic event construction, and write comprehensive tests covering: all 29 data models, all 29 typed event classes, discriminated union parsing, ULID validation, extra="forbid" enforcement, mode field filtering, deterministic event construction, and smoke tests for import/type consistency. This plan follows TDD pattern — write tests first, then implement the factory helper.
</objective>

<read_first>
  - /Users/tmac/Projects/state/src/state_core/schema.py (current state after Plan B — all models + discriminated unions)
  - /Users/tmac/Projects/state/tests/test_schema.py (existing tests from Phase 001)
  - /Users/tmac/Projects/state/.planning/milestones/v1/phases/002-pydantic-event-schema-all-28/CONTEXT.md
  - /Users/tmac/Projects/state/.planning/research/ARCHITECTURE.md (§5.2 dual-write contract, §5.3 event taxonomy)
</read_first>

---

### Task 1: Write comprehensive test suite for all schemas

<action>
Replace the existing `tests/test_schema.py` with the following comprehensive test file. This tests every model, every typed event, discriminated union parsing, ULID validation, `extra="forbid"`, and mode filtering.
...
class TestModeDecisionAuthDataModels:
    ...
    def test_all_data_models_reject_extra(self) -> None:
        ...
        for model_cls, kwargs in [
            ...
            # Sanity: original object is valid
...
class TestTypedEvents:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (SliceWorktreeReadyEvent, SliceWorktreeReadyData(worktree_name="w", branch="b", dir="/x"), "state.slice.worktree_ready", "slice"),
            ...
            (ConceptIntroducedEvent, ConceptIntroducedData(concept_id="c", name="N", prerequisites=[]), "state.concept.introduced", "concept"),
            (ConceptObservedEvent, ConceptObservedData(observation="O", classification="C"), "state.concept.observed", "concept"),
            ...
            (ModeActivatedEvent, ModeActivatedData(mode_value="build"), "state.mode.activated", "mode"),
...
class TestEdgeCases:
    ...
    def test_negative_seq(self) -> None:
        ...
        e = EventEnvelope(seq=-1)  # schema doesn't constrain this
        assert e.seq == -1
...
class TestImports:
    ...
    def test_discriminated_union_names_exist(self) -> None:
        ...
        from src.state_core.schema import (
            AnyStateEvent, ArcEvent, AuthEvent, ConceptEvent,
            DecisionEvent, DrillEvent, ModeEvent, PhaseEvent,
            SliceEvent, StepEvent,
        )
...
<acceptance_criteria>
  - `uv run python3 -m pytest tests/test_schema.py -v --tb=short` — all tests pass.
  - Test count is at least 50 (covering all data models, typed events, discriminated unions, validations).
  - `grep -c "def test_" tests/test_schema.py` returns a count ≥ 50.
  - `ruff check tests/test_schema.py --no-cache` exits 0.
</acceptance_criteria>

---

### Task 2: Add `build_event()` factory helper to schema.py

<action>
Append the following factory function to `src/state_core/schema.py` (at the very end of the file). This helper provides ergonomic event construction with automatic ULID generation, timestamp injection, and type-safe construction.
...
def build_event(
    ...
    *,
    ...
    if id_ is None:
        id_ = str(ulid())
    if ts is None:
        # Use fixed deterministic timestamp when not provided.
        # Production callers should always pass an explicit ts.
...
Note: The `ulid` import needs to be at the top of the file. Also `TypeVar` needs to be imported. Add these to the existing imports:

1. Add `import ulid` alongside existing imports (after `from __future__ import annotations` block)
2. Add `TypeVar` to the `typing` import: `from typing import Annotated, Any, Literal, TypeVar, Union`
</action>

<acceptance_criteria>
  - `uv run python3 -c "
from src.state_core.schema import build_event, ArcCreatedEvent, ArcCreatedData
e = build_event(ArcCreatedEvent, 'arc-01', ArcCreatedData(title='Test', goal='Learn'), mode='build')
assert isinstance(e, ArcCreatedEvent)
assert e.aggregate_id == 'arc-01'
assert e.data.title == 'Test'
assert e.mode == 'build'
assert len(e.id) == 26  # ULID length
print(f'Event {e.id}: {e.type} for {e.aggregate_id}')
"` — factory creates valid events with auto-generated ULIDs.
  - `uv run python3 -c "
from src.state_core.schema import build_event, StepVerifyPassedEvent, StepVerifyPassedData
e = build_event(StepVerifyPassedEvent, 'step-1', StepVerifyPassedData(duration_ms=100), mode='build', id_='01ARZ3NDEKTSV4RRFFQ69G5FAV')
assert e.id == '01ARZ3NDEKTSV4RRFFQ69G5FAV'
print('Custom ULID override works')
"` — custom id_ override works.
  - `uv run python3 -c "
from src.state_core.schema import build_event, ArcCreatedEvent, ArcCreatedData
e1 = build_event(ArcCreatedEvent, 'arc-01', ArcCreatedData(title='T', goal='G'))
e2 = build_event(ArcCreatedEvent, 'arc-01', ArcCreatedData(title='T', goal='G'))
assert e1.id != e2.id  # each call gets a unique ULID
print('Unique ULIDs OK')
"` — each call gets a unique ULID.
  - `ruff check src/state_core/schema.py --no-cache` exits 0.
</acceptance_criteria>

---

### Task 3: Add factory-specific tests

<action>
Append the following test class to `tests/test_schema.py` (before the `TestImports` class at the bottom):
...
<acceptance_criteria>
  - `uv run python3 -m pytest tests/test_schema.py -v --tb=short -k "TestBuildEvent"` — all 8 factory tests pass.
  - `uv run python3 -m pytest tests/test_schema.py -v --tb=short` — all tests pass (≥58 tests).
  - `ruff check tests/test_schema.py --no-cache` exits 0.
</acceptance_criteria>
...
<verification>
1. **Full test suite:** `uv run python3 -m pytest tests/ -v --tb=short` — all tests pass.
2. **Factory smoke test:** `uv run python3 -c "
from src.state_core.schema import build_event, ArcCreatedEvent, ArcCreatedData
e = build_event(ArcCreatedEvent, 'arc-01', ArcCreatedData(title='T', goal='G'), mode='build')
print(f'Created: {e.type} [{e.id}]')"` — factory works.
3. **Lint:** `ruff check src/state_core/tests/ --no-cache` — exits 0.
4. **Type check:** `uv run mypy src/state_core/schema.py --strict` — no type errors.
5. **Determinism assertion:** `uv run python3 -c "
from src.state_core.schema import build_event, ArcCreatedEvent, ArcCreatedData
# Without explicit id_ and ts, two factories produce different ids (nondeterministic by design)
# But with same params, the same explicit result should be reproducible
e1 = build_event(ArcCreatedEvent, 'a1', ArcCreatedData(title='T', goal='G'), mode='build', id_='01ARZ3NDEKTSV4RRFFQ69G5FAV', ts='2026-01-01T00:00:00Z')
e2 = build_event(ArcCreatedEvent, 'a1', ArcCreatedData(title='T', goal='G'), mode='build', id_='01ARZ3NDEKTSV4RRFFQ69G5FAV', ts='2026-01-01T00:00:00Z')
assert e1.model_dump_json() == e2.model_dump_json()
print('Determinism OK with explicit params')"` — deterministic with explicit params.
6. **Module exports useful names:** `uv run python3 -c "
from src.state_core.schema import build_event, EventEnvelope, AnyStateEvent, ArcEvent
print('Factory and unions importable')"`
</verification>

<must_haves>
- **Comprehensive test suite:** ≥58 tests covering all data models, typed events, discriminated unions, ULID validation, mode filtering, determinism, edge cases
- **`build_event()` factory function** with auto-generated ULID, optional id/ts/seq overrides, type-safe construction
- **All existing tests continue to pass** (no regressions from Phase 001)
- **ruff clean, mypy clean** on both `schema.py` and `test_schema.py`
- **`python-ulid>=3.0` dependency** correctly installed and importable
</must_haves>
